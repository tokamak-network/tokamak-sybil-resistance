...
def Sigma(input) :
    return pow(input, 5, mod)


def Ark(t, C, r, input) :
    out = [0 for i in range(t)]
    for i in range(t) :
        out[i] = (input[i] + C[i + r]) % mod
    return out
...
def MixS(t, S, r, input) :
    ...
    lc = 0
    for i in range(t) :
        lc = (lc + S[(t*2-1)*r+i]*input[i]) % mod
        
    out[0] = lc
    for i in range(1,t) :
        out[i] = (input[i] +  input[0] * S[(t*2-1)*r + t + i - 1]) % mod
    return out
...
def PoseidonEx(nInputs, nOuts, inputs, initialState) :
    ...
    for r in range(nRoundsP) :
        ...
        for j in range(t) :
            if j == 0 :
                input[j] = (sigmaP[r] + C[(nRoundsF//2+1)*t + r]) % mod
            else :
                if r == 0 :
