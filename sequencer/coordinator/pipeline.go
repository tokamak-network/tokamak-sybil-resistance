...
func NewPipeline(ctx context.Context,
	...
	if proversPoolSize == 0 {
		return nil, common.Wrap(fmt.Errorf("no provers in the pool"))
	}
	return &Pipeline{
		num:                   num,
		cfg:                   cfg,
...
func (p *Pipeline) reset(batchNum common.BatchNum,
	...
	if common.Unwrap(err) == sql.ErrNoRows {
		...
		localStateRoot := p.batchBuilder.LocalStateDB().AccountTree.Root().BigInt()
...
// determine by policy if we should forge the batch or not.  Returns true and
// the reason when the forging of the batch must be skipped.
func (p *Pipeline) forgePolicySkipPostSelection(now time.Time, l1UserTxsExtra []common.L1Tx,
	...
	pendingTxs := true
	if len(l1UserTxsExtra) == 0 && len(poolL2Txs) == 0 {
...
func (p *Pipeline) forgeBatch(batchNum common.BatchNum) (batchInfo *BatchInfo,
	...
	if p.shouldL1L2Batch(batchInfo) {
		...
		l1UserTxs, poolL2Txs, discardedL2Txs, err =
	...
	if skip, reason, err := p.forgePolicySkipPostSelection(now,
		l1UserTxs, poolL2Txs, batchInfo); err != nil {
		return nil, nil, common.Wrap(err)
	} else if skip {
