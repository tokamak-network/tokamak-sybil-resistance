    ...
    OwnableUpgradeable
{
    ...
    function setUp() public {
        ...
        address currentOwner = sybil.owner(); // Get the owner of the contract
        emit log_address(currentOwner); // Log the current owner
    }

    function testGetNextL1ToForgeQueue() public {
        vm.prank(0x0000000000000000000000000000000000000000);
        sybil.setNextL1ToForgeQueue(5);
        uint32 nextL1ToForgeQueue = sybil.getNextL1ToForgeQueue();
        assertEq(nextL1ToForgeQueue, 5);
    }

    function testGetNextL1FillingQueue() public {
        vm.prank(0x0000000000000000000000000000000000000000);

        sybil.setNextL1FillingQueue(10);
        uint32 nextL1FillingQueue = sybil.getNextL1FillingQueue();
        assertEq(nextL1FillingQueue, 10);
    }

    function testGetLastIdx() public {
        vm.prank(0x0000000000000000000000000000000000000000);

        sybil.setLastIdx(200);
        uint48 lastIdx = sybil.getLastIdx();
        assertEq(lastIdx, 200);
    }

    function testGetLastL1L2Batch() public {
        vm.prank(0x0000000000000000000000000000000000000000);

        sybil.setLastL1L2Batch(999);
        uint64 lastL1L2Batch = sybil.getLastL1L2Batch();
        assertEq(lastL1L2Batch, 999);
    }

    function testGetterStateRoot() public {
        vm.prank(0x0000000000000000000000000000000000000000);

        sybil.setStateRoot(1, 0xabc123);
        uint256 stateRoot = sybil.getStateRoot(1);
        assertEq(stateRoot, 0xabc123);
    }

    function testGetVouchRoot() public {
        vm.prank(0x0000000000000000000000000000000000000000);

        sybil.setVouchRoot(1, 0xabc456);
        uint256 vouchRoot = sybil.getVouchRoot(1);
        assertEq(vouchRoot, 0xabc456);
    }

    function testGetScoreRoot() public {
        vm.prank(0x0000000000000000000000000000000000000000);

        sybil.setScoreRoot(1, 0xabc789);
        uint256 scoreRoot = sybil.getScoreRoot(1);
        assertEq(scoreRoot, 0xabc789);
    }

    function testGetExitRoot() public {
        vm.prank(0x0000000000000000000000000000000000000000);

        sybil.setExitRoot(1, 0xdeadbeef);
        uint256 exitRoot = sybil.getExitRoot(1);
        assertEq(exitRoot, 0xdeadbeef);
    }
    ...
    function testGetL1TransactionQueue() external {
        ...
        vm.prank(address(this));
        sybil.addL1Transaction{value: loadAmount}(
            babyPubKey,
            fromIdx,
            loadAmountF,
            amountF,
        ...
        bytes32 slot = keccak256(
            abi.encode(
                ...
                keccak256(
                    abi.encode(
                        numExitRoot,
                        uint256(keccak256("exitNullifierMap"))
                    )
                )
            )
        );
    ...
    function testWithdrawMerkleProofInvalidSmtProof() public {
        ...
        bytes32 slot = keccak256(
            abi.encode(numExitRoot, uint256(keccak256("exitRootsMap")))
        );
    ...
    function testWithdrawMerkleProofTransferFails() public {
        ...
        // Directly set exitRootsMap[numExitRoot] to a dummy value
        bytes32 exitRootSlot = keccak256(
            abi.encode(numExitRoot, uint256(keccak256("exitRootsMap")))
        );
    ...
    function testWithdrawMerkleProofTransferPasses() public {
        ...
        */
        ...
        // verify proof
        bool isVerified = verify(siblings, exitRoot, leaf, idx);
    ...
    function calculateTestExitTreeRoot() internal returns (bytes32) {
        ...
        while (n > 0) {
            for (uint256 i = 0; i < n - 1; i += 2) {
                uint256 res = sybil._hashNode(
                    uint(hashes[offset + i]),
                    uint(hashes[offset + i + 1])
                );
                hashes.push(bytes32(res));
            }
            offset += n;
            n = n / 2;
        }
